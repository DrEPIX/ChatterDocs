...
app.post('/api/rooms', (req, res) => {
  try {
    ...
    const { name, type, description, minecraft_host, minecraft_port, minecraft_version, resource_pack } = req.body;
    if (!name || !type) return res.status(400).json({ error: 'Missing fields' });
    const id = nanoid(12);
    db.prepare(
      `INSERT INTO rooms (id, name, type, description, minecraft_host, minecraft_port, minecraft_version, resource_pack)
       VALUES (@id, @name, @type, @description, @minecraft_host, @minecraft_port, @minecraft_version, @resource_pack)`
    ).run({ id, name, type, description, minecraft_host, minecraft_port, minecraft_version, resource_pack });
...
app.put('/api/rooms/:id', (req, res) => {
  try {
    ...
    const { name, type, description, minecraft_host, minecraft_port, minecraft_version, resource_pack } = req.body;
    db.prepare(
      `UPDATE rooms SET name=@name, type=@type, description=@description, minecraft_host=@minecraft_host, minecraft_port=@minecraft_port, minecraft_version=@minecraft_version, resource_pack=@resource_pack WHERE id=@id`
    ).run({ id, name, type, description, minecraft_host, minecraft_port, minecraft_version, resource_pack });
